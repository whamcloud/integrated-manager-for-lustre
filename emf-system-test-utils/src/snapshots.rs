...
impl From<&String> for SnapshotName {
    fn from(s: &String) -> Self {
        match s.to_lowercase().as_str() {
            ...
            "emf-configured" => Self::EmfConfigured,
...
impl fmt::Display for SnapshotName {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            Self::EmfConfigured => write!(f, "emf-configured"),
...
pub fn get_snapshot_name_for_state(config: &Config, state: TestState) -> snapshots::SnapshotName {
    match state {
        ...
        TestState::Configured => SnapshotName::EmfConfigured,
...
pub fn create_graph(snapshots: &[SnapshotName]) -> DiGraph<Snapshot, Transition> {
    ...
    let emf_configured = graph.add_node(Snapshot {
        name: SnapshotName::EmfConfigured,
        available: snapshots.contains(&SnapshotName::EmfConfigured),
    });
    ...
    graph.add_edge(
        ...
        Transition {
            path: SnapshotPath::All,
            transition: mk_transition(configure_emf),
        },
    );
