...
use emf_graphql_queries::task;
use emf_wire_types::{task::KeyValue, task::TaskArgs, Branding, FsType};
use futures::future::try_join_all;
use ssh::create_emf_diagnostics;
...
impl From<std::io::Error> for TestError {
    fn from(err: std::io::Error) -> Self {
        TestError::CmdError(emf_cmd::CmdError::Io(err))
    }
}
...
impl<T: Into<TestError> + Send> WithSos for Result<Config, T> {
    async fn handle_test_result(self, hosts: Vec<&str>, prefix: &str) -> Result<Config, TestError> {
        create_emf_diagnostics(hosts, prefix).await?;
...
pub async fn wait_on_services_ready(config: &Config) -> Result<(), TestError> {
    match config.test_type {
        TestType::Rpm => {
            let (_, output) =
            ssh::ssh_exec(config.manager_ip, "systemctl list-dependencies emf-manager.target | tail -n +2 | awk '{print$2}' | awk '{print substr($1, 3)}' | grep -v emf-settings-populator.service | grep -v emf-sfa.service").await?;
        ...
        TestType::Docker => {
            let mut cmd = ssh::systemd_status(config.manager_ip, "emf-docker.service").await?;
            try_command_n_times(50, 5, &mut cmd).await?;
        }
    };
...
pub async fn setup_bare(config: Config) -> Result<Config, TestError> {
    ...
    match config.test_type {
        TestType::Rpm => match env::var("REPO_URI") {
            Ok(x) => {
                vagrant::provision_node(config.manager, "install-emf-repouri")
                    .await?
                    .env("REPO_URI", x)
                    .checked_status()
                    .await?;
            ...
            _ => {
                vagrant::provision_node(config.manager, "install-emf-local")
                    .await?
                    .checked_status()
                    .await?;
            }
        ...
        TestType::Docker => {
            ...
            match env::var("REPO_URI") {
                Ok(x) => {
                    vagrant::provision_node(config.manager, "install-emf-docker-repouri")
                        .await?
                        .env("REPO_URI", x)
                        .checked_status()
                        .await?;
                ...
                _ => {
                    vagrant::provision_node(config.manager, "install-emf-docker-local")
                        .await?
                        .checked_status()
                        .await?;
                }
...
pub async fn configure_emf(config: Config) -> Result<Config, TestError> {
    vagrant::up()
        .await?
...
pub async fn install_fs(config: Config) -> Result<Config, TestError> {
    ...
    ssh::install_ldiskfs_no_emf(&config).await?;
...
pub async fn test_stratagem_taskqueue(config: Config) -> Result<Config, TestError> {
    ...
    if let Ok((_, output)) = ssh::ssh_exec(
        config.manager_ip,
        "emf debugapi get report/test-taskfile.txt",
    )
    .await
    {
...
pub async fn configure_rpm_setup(config: &Config) -> Result<(), TestError> {
    ...
    let vm_cmd = format!(
        "mkdir -p /var/lib/chroma && sudo cp /vagrant/overrides.conf /var/lib/chroma {} \
         && sudo systemctl restart emf-manager.target",
        path_list
            .into_iter()
...
pub async fn configure_docker_setup(config: &Config) -> Result<(), TestError> {
    ...
    let vm_cmd = format!(
        "sudo mkdir -p /etc/emf-docker/setup && sudo cp /vagrant/config /etc/emf-docker/setup/ {}",
        path_list
            .into_iter()
            .map(|p| format!(" && sudo cp /vagrant/{} /etc/emf-docker/setup/", p))
            .collect::<Vec<String>>()
            .concat()
    );
