...
pub async fn get_conn(pool: Pool) -> Result<Connection, ImlRabbitError> {
...
) -> Result<impl Stream<Item = Result<(Channel, message::Delivery), ImlRabbitError>>, ImlRabbitError>
{
    let options = options.unwrap_or_default();
...
pub async fn basic_consume_one(
    ...
    consumer_tag: impl Into<String>,
    options: Option<BasicConsumeOptions>,
) -> Result<(Channel, message::Delivery), ImlRabbitError> {
    let options = options.unwrap_or_default();
...
mod tests {
    ...
    #[tokio::test]
    async fn test_connection_filter() -> Result<(), ImlRabbitError> {
