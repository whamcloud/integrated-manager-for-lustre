...
use emf_manager_client::{get_client, get_retry, Client, EmfManagerClientError};
...
pub trait ToApiRecord: std::fmt::Debug + Id {
    fn to_api_record<T: 'static>(
        &self,
        client: Client,
    ) -> Pin<Box<dyn Future<Output = Result<T, EmfManagerClientError>> + Send>>
    where
...
) -> Result<RecordChange, EmfManagerClientError>
where
    T: std::fmt::Debug
    ...
    match (msg_type, &x) {
        ...
        (MessageType::Insert, x) | (MessageType::Update, x) => {
            ...
            match r {
                ...
                Err(EmfManagerClientError::Reqwest(ref e))
                    if e.status() == Some(emf_manager_client::StatusCode::NOT_FOUND) =>
                {
...
pub async fn populate_from_api(shared_api_cache: SharedCache) -> Result<(), EmfManagerClientError> {
