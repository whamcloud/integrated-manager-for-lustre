...
class EventResource(ModelResource):
    """
    Events are messages generated by Chroma to represent what
    changes on hosts or filesystems it monitors.  Events are essentially
    ...
    host_name = fields.CharField(attribute = 'host', blank = True, null = True,
            help_text = "The ``label`` attribute of the host on which the event\
                    occurred, or null if the event is not specific to a single\
                    host")
    host = fields.ToOneField('chroma_api.host.HostResource', 'host', null = True,
            help_text = "The host on which the event occurred, or null if the event\
