...
pub async fn process_retention(
    ...
    for fs_name in filesystems {
        ...
        if let Some((bytes_total, bytes_free, bytes_used)) = stats {
            ...
            let percent_used = bytes_used as f64 / bytes_total as f64 * 100.0f64;
            ...
            if let Some(retention) = retention {
                ...
                let delete_snapshot = match retention.reserve_unit {
                ...
                tracing::error!("Delete snapshot?: {}", delete_snapshot);

                if delete_snapshot
                    && !snapshots.is_empty()
                    && snapshots.len() > retention.keep_num.unwrap_or_default() as usize
