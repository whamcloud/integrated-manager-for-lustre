...
pub async fn get_db_device_for_fqdn(
    mut client: &mut Client,
    fqdn: &Fqdn,
) -> Result<Devices, iml_postgres::Error> {
    select_all(
        &mut client,
        &format!(
            "SELECT * FROM {} WHERE fqdn={}",
            Device::table_name(),
            fqdn.0,
        ),
        iter::empty(),
    ...
    for c in change::get_changes_values(&local_db_devices, &incoming_devices.iter().collect()) {
        match c {
            Change::Add(d) => {
                ...
                transaction
                    .execute(
                        "INSERT INTO chroma_core_device (id, size, usable_for_lustre, device_type, parents, children) VALUES ($1, $2, $3, $4, $5, $6, $7)",
                        &[
                            &d.id,
                            &d.size,
                            &d.usable_for_lustre,
            ...
            Change::Update(d) => {
                ...
                transaction
                    .execute(
                        "UPDATE chroma_core_device SET size = $2, usable_for_lustre = $3, device_type = $4, parents=$5, children=$6 WHERE id = $1",
                        &[
                            &d.id,
                            &d.size,
                            &d.usable_for_lustre,
...
pub async fn persist_local_device<'a>(
    transaction: &mut Transaction<'a>,
    fqdn: &Fqdn,
    incoming_device: &device_types::devices::Device,
) -> Result<(), ImlDeviceError> {
    tracing::debug!("Going to add device from host {}", fqdn);
