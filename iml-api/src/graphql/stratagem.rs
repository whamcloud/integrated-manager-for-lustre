...
use crate::{
    command::get_command,
    error::ImlApiError,
    graphql::{authorize, fs_id_by_name, insert_task, Context, SendJob},
};
use futures::{
...
impl StratagemQuery {
    ...
    async fn stratagem_reports(context: &Context) -> juniper::FieldResult<Vec<StratagemReport>> {
        if authorize(
            &context.enforcer,
            &context.session,
            "query::stratagem::stratagem_reports",
        )? {
...
impl StratagemMutation {
    async fn run_filesync(
        ...
        if authorize(
            &context.enforcer,
            &context.session,
            "mutation::stratagem::run_filesync",
        )? {
    ...
    async fn run_cloudsync(
        ...
        if authorize(
            &context.enforcer,
            &context.session,
            "mutation::stratagem::run_cloudsync",
        )? {
            let uuid = Uuid::new_v4().to_hyphenated().to_string();
    ...
    async fn run_fast_file_scan(
        ...
        if authorize(
            &context.enforcer,
            &context.session,
            "mutation::stratagem::run_fast_file_scan",
        )? {
    ...
    async fn run_stratagem(
        ...
        if authorize(
            &context.enforcer,
            &context.session,
            "mutation::stratagem::run_stratagem",
        )? {
    ...
    async fn delete_stratagem_report(
        ...
        if authorize(
            &context.enforcer,
            &context.session,
            "mutation::stratagem::delete_stratagem_report",
        )? {
