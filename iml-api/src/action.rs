...
struct ApiTransition<'a>(&'a CompositeId, Transition);

impl<'a> From<ApiTransition<'a>> for AvailableAction {
    fn from(x: ApiTransition) -> Self {
        AvailableAction {
            args: None,
            composite_id: x.0.to_string(),
            class_name: None,
            confirmation: None,
...
struct ApiJob<'a>(&'a CompositeId, Job);

impl<'a> From<ApiJob<'a>> for AvailableAction {
