...
use iml_change::{Deletions, Upserts};
use iml_postgres::sqlx;
use iml_wire_types::sfa::wbem_interop::{SfaJob, SfaJobRow};
use unzip_n::unzip_n;

unzip_n!(6);

pub async fn all(pool: &sqlx::PgPool) -> Result<Vec<SfaJob>, ImlSfaError> {
...
pub async fn batch_upsert(x: Upserts<&SfaJob>, pool: sqlx::PgPool) -> Result<(), ImlSfaError> {
...
pub async fn batch_delete(xs: Deletions<&SfaJob>, pool: sqlx::PgPool) -> Result<(), ImlSfaError> {
