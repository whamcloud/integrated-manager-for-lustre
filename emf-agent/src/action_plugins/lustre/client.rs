...
use emf_fs::read_file_to_end;
use emf_wire_types::client::{Mount, Unmount};
use std::io;
use tokio::{
    fs::{self, File},
    io::AsyncWriteExt,
...
static FSTAB_EDIT: &str = "/etc/fstab.emf.edit";
...
async fn is_mounted(mountpoint: &str, check_fstab: bool) -> Result<bool, EmfAgentError> {
    ...
    match output.status.code() {
        ...
        Some(1) => Ok(false),
        Some(code) => Err(EmfAgentError::Io(io::Error::new(
            io::ErrorKind::Other,
            format!("is_mounted: {} exited with code: {}", mountpoint, code),
        ))),
        None => Err(EmfAgentError::Io(io::Error::new(
            io::ErrorKind::Other,
...
pub async fn mount(mount: Mount) -> Result<(), EmfAgentError> {
...
pub async fn mount_many(xs: Vec<Mount>) -> Result<(), EmfAgentError> {
    for x in xs {
        mount(x).await?;
    }
...
pub async fn unmount(unmount: Unmount) -> Result<(), EmfAgentError> {
...
/// - Add a systemd mount to `/etc/fstab`
/// - Optionally set systemd mount to start at boot iff `Mount.persist` is `true`.
pub async fn add_fstab_entry(mount: Mount) -> Result<(), EmfAgentError> {
    // This should return Ok(()) if the dir already exists.
    fs::create_dir_all(&mount.mountpoint).await?;
...
pub async fn remove_fstab_entry(mount: Mount) -> Result<(), EmfAgentError> {
    delete_fstab_entry(&mount.mountpoint).await?;

    Ok(())
}

pub async fn unmount_many(xs: Vec<Unmount>) -> Result<(), EmfAgentError> {
    for x in xs {
        unmount(x).await?;
    }
...
async fn get_fstab() -> Result<String, EmfAgentError> {
...
async fn write_fstab(contents: &str) -> Result<(), EmfAgentError> {
...
async fn delete_fstab_entry(mountpoint: &str) -> Result<(), EmfAgentError> {
