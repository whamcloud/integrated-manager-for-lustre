...
async fn write_to_file(content: String) -> Result<(), EmfAgentError> {
...
async fn read_ldev_config() -> Result<String, EmfAgentError> {
    let ldev_path = env::get_ldev_conf();
...
pub async fn create(entries: Vec<LdevEntry>) -> Result<(), EmfAgentError> {
    if !entries.is_empty() {
        ...
        Err(EmfAgentError::LdevEntriesError(
...
mod tests {
    use super::*;
    use emf_wire_types::FsType;

    #[test]
    fn test_create() -> Result<(), EmfAgentError> {
        let entries = vec![
            LdevEntry {
                primary: "mds1".into(),
    ...
    #[test]
    fn test_config_without_ha() -> Result<(), EmfAgentError> {
        let entries = vec![
            LdevEntry {
                primary: "mds1".into(),
                failover: None,
