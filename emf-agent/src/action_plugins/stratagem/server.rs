...
use crate::{agent_error::EmfAgentError, agent_error::RequiredError, http_comms::streaming_client};
use emf_cmd::{CheckedCommandExt, Command};
use emf_fs::{read_file_to_end, stream_dir_lines, write_tempfile};
use emf_wire_types::stratagem::{StratagemConfig, StratagemDevice, StratagemGroup, StratagemRule};
...
) -> Result<MailboxFiles, EmfAgentError> {
    stratagem_result
        .group_counters
        .iter()
            ...
            group
                ...
                .map(move |(idx, counter)| {
                    let group = stratagem_data
                        ...
                        .ok_or_else(|| {
                            EmfAgentError::RequiredError(RequiredError(format!(
...
) -> Result<(String, StratagemResult, MailboxFiles), EmfAgentError> {
...
pub async fn stream_fidlists(mailbox_files: MailboxFiles) -> Result<(), EmfAgentError> {
