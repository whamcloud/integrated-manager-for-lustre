...
async fn read_corosync_conf() -> Result<Vec<u8>, io::Error> {
    emf_fs::read_file_to_end(COROSYNC_CONF).await
}

/// Truncates and writes a new configuration.
async fn write_corosync_conf(xs: &[u8]) -> Result<(), EmfAgentError> {
    OpenOptions::new()
        .write(true)
        .truncate(true)
        .open(COROSYNC_CONF)
...
pub(crate) async fn change_mcast_port(new_port: u16) -> Result<(), EmfAgentError> {
...
async fn corosync_cmapctl() -> Result<(Totem, Vec<Interface>), EmfAgentError> {
...
fn corosync_cmapctl_parser(x: &[u8]) -> Result<(Totem, Vec<Interface>), EmfAgentError> {
...
fn try_get<'a>(hm: &'a StringMap<'a>, k: &str) -> Result<&'a &'a str, EmfAgentError> {
    hm.get(k)
