    ...
    def metrics_fetch(self,fs,target_kind,fetch_metrics,start_time,end_time,interval,datafunction='Average'):
        ...
        if target_kind == 'OST':
            if start_time:
                ...
                if fetch_metrics:
                    fs_target_stats = fs.metrics.fetch(datafunction,ObjectStoreTarget,fetch_metrics=fetch_metrics.split(),start_time=int(time.time()-start_time))
                else:
                    fs_target_stats = fs.metrics.fetch(datafunction,ObjectStoreTarget,start_time=int(time.time()-start_time))
            else:
                if fetch_metrics:
            ...
            if start_time:
                start_time = int(start_time)
                if fetch_metrics:
                    fs_target_stats = fs.metrics.fetch(datafunction,MetadataTarget,fetch_metrics=fetch_metrics.split(),start_time=int(time.time()-start_time))
                else:
                    fs_target_stats = fs.metrics.fetch(datafunction,MetadataTarget,start_time=int(time.time()-start_time))
            else:
                if fetch_metrics:
...
class GetFSServerStats(AnonymousRequestHandler):
    ...
    def run(self,request,filesystem,starttime,endtime ,datafunction,fetchmetrics):
        ...
        if filesystem:
            ...
            for host in hosts:
                host_stats_metric.append(self.metrics_fetch(host,fetchmetrics,starttime,endtime,interval))
            ...
            for fs in Filesystem.objects.all():
                ...
                for host in Host.objects.all():
                    host_stats_metric.append(self.metrics_fetch(host,fetchmetrics,starttime,endtime,interval))
                all_host_stats_metrics.extend(host_stats_metric)  
    ...
    def metrics_fetch(self,host,fetch_metrics,start_time,end_time,interval,datafunction='Average'):
        ...
        if start_time:
            start_time = int(start_time)
            if fetch_metrics:
                host_stats = host.metrics.fetch(datafunction,fetch_metrics=fetch_metrics.split(),start_time=int(time.time()-start_time))
            else:
                host_stats = host.metrics.fetch(datafunction,start_time=int(time.time()-start_time))
        else:
            if fetch_metrics:
...
class GetFSMGSStats(AnonymousRequestHandler):
    ...
    def metrics_fetch(self,mgs,fetch_metrics,start_time,end_time,interval,datafunction='Average'):
        ...
        if start_time:
            start_time = int(start_time)
            if fetch_metrics:
                mgs_stats = mgs.metrics.fetch(datafunction,fetch_metrics=fetch_metrics.split(),start_time=int(time.time()-start_time))
            else:
                mgs_stats = mgs.metrics.fetch(datafunction,start_time=int(time.time()-start_time))
        else:
            if fetch_metrics:
...
class GetTargetStats(AnonymousRequestHandler):
    ...
    def metrics_fetch(self,target,fetch_metrics,start_time,end_time,interval,datafunction='Average'):
        ...
        if start_time:
            start_time = int(start_time)
            if fetch_metrics:
                target_stats = target.metrics.fetch(datafunction,fetch_metrics=fetch_metrics.split(),start_time=int(time.time()-start_time))
            else:
                target_stats = target.metrics.fetch(datafunction,start_time=int(time.time()-start_time))
        else:
            if fetch_metrics:
...
class GetFSClientsStats(AnonymousRequestHandler):
    ...
    def metrics_fetch(self,filesystem,start_time,end_time,interval,datafunction='Average'):
        ...
        if start_time:
            start_time = int(start_time)
            client_stats = filesystem.metrics.fetch(datafunction,ObjectStoreTarget,fetch_metrics=fetch_metrics.split(),start_time=int(time.time()-start_time))
        else:
            try:
