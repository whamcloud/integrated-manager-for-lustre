...
class ConfigStore(object):
    ...
    def sections(self):
        with self._lock:
            return sorted(
                [os.path.basename(e) for e in os.listdir(self.path) if os.path.isdir(os.path.join(self.path, e))]
            )
    ...
    def get_section(self, section):
        with self._lock:
            return dict([(key, self.get(section, key)) for key in self.get_section_keys(section)])

    def get_all(self):
        with self._lock:
            return dict([(section, self.get_section(section)) for section in self.sections])
            ...
            try:
                ...
                if e.errno == errno.ENOENT:
                    if not section in self.sections:
                        ...
                        raise KeyError("Invalid key '%s' for config section '%s'" % (key, section))
                raise
