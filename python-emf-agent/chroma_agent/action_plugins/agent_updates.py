...
from emf_common.lib.agent_rpc import agent_result, agent_error, agent_result_ok
...
def restart_agent():
    def _shutdown():
        daemon_log.info("Restarting emf-storage-server.target")
        # Use subprocess.Popen instead of try_run because we don't want to
        # wait for completion.
        subprocess.Popen(["systemctl", "restart", "emf-storage-server.target"])
