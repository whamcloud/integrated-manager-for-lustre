...
class PacemakerNode(object):
    ...
    def fence_agent_dicts(self):
        ...
        for agent in [k for k in self.attributes if "agent" in k]:
            ...
            agents.append(dict([t for t in self.attributes.items() if t[0].startswith("%s_fence_" % index)]))
...
class PacemakerConfig(object):
    ...
    def resource_list(self):
        try:
            return AgentShell.try_run(["crm_resource", "--list-raw"]).strip().split("\n")
        except AgentShell.CommandExecutionError:
            return []

    def get_resource(self, label):
        try:
            res = AgentShell.try_run(["crm_resource", "--query-xml", "--resource", label]).strip()
        except AgentShell.CommandExecutionError:
            return None
        i = res.find("\nxml:\n")
        if not i:
    ...
    @property
    def configured(self):
        """configured returns True if this node has a pacemaker configuration set by EMF.
