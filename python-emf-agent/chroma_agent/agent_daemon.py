...
def main():
    ...
    parser = argparse.ArgumentParser(description="EXAScaler Management Framework software Agent")
    ...
    try:
        ...
        try:
            url = urljoin(os.environ["EMF_MANAGER_URL"], "agent/message/")
        except KeyError as e:
            daemon_log.error(
                "No configuration found (must be registered before running the agent service), " "details: %s" % e
            )
            return
