...
from emf_common.lib.exception_sandbox import exceptionSandBox
...
class CorosyncPlugin(DevicePlugin):
    ...
    def _parse_crm_as_xml(self, raw):
        ...
        try:
            ...
            tm_datetime = EMFDateTime.strptime(tm_str, "%a %b %d %H:%M:%S %Y")
            return_dict.update(
                {"datetime": EMFDateTime.convert_datetime_to_utc(tm_datetime).strftime("%Y-%m-%dT%H:%M:%S+00:00")}
            )
