...
%description
This is the EXAScaler Management Framework monitoring and adminstration agent
...
Requires:       emf-device-scanner >= 4.0.0
...
This package layers on management capabilities for EXAScaler Management Framework Agent.
...
install -m 644 emf-storage-server.target %{buildroot}%{_unitdir}/emf-storage-server.target
...
%systemd_post emf-storage-server.target
...
%attr(0644,root,root)%{_unitdir}/emf-storage-server.target
...
- Pin mimimum EMF version requirements
