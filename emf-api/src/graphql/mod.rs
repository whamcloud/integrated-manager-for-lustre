...
use emf_rabbit::{EmfRabbitError, Pool};
use emf_wire_types::{
...
async fn get_fs_target_resources(
    pool: &PgPool,
    fs_name: Option<String>,
) -> Result<Vec<TargetResource>, EmfApiError> {
...
async fn get_banned_targets(pool: &PgPool) -> Result<Vec<BannedTargetResource>, EmfApiError> {
    let xs = sqlx::query!(
        r#"
...
async fn run_jobs<T: std::fmt::Debug + serde::Serialize>(
    ...
    rabbit_pool: &Pool,
) -> Result<i32, EmfApiError> {
    let kwargs: HashMap<String, String> = vec![("message".into(), msg.to_string())]
        .into_iter()
        .collect();

    let id: i32 = emf_job_scheduler_rpc::call(
        &rabbit_pool.get().await.map_err(EmfRabbitError::PoolError)?,
        "run_jobs",
        vec![jobs],
        Some(kwargs),
    )
