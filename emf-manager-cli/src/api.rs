...
pub async fn api_cli(command: ApiCommand) -> Result<(), EmfManagerCliError> {
    let uri = emf_manager_client::create_api_url(command.path)?;

    api_command(command.method, uri, command.body).await
}

pub async fn graphql_cli(command: GraphQlCommand) -> Result<(), EmfManagerCliError> {
    let uri = emf_manager_client::create_url("/graphql")?;
