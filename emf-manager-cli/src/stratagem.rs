...
fn parse_duration(src: &str) -> Result<u64, EmfManagerCliError> {
    if src.len() < 2 {
...
async fn report_cli(cmd: ReportCommand) -> Result<(), EmfManagerCliError> {
    match cmd {
        ReportCommand::List { display_type } => {
            let query = stratagem_queries::list_reports::build();

            let resp: emf_graphql_queries::Response<stratagem_queries::list_reports::Resp> =
                graphql(query).await?;
        ...
        ReportCommand::Delete { name } => {
            for n in name {
                ...
                let _resp: emf_graphql_queries::Response<stratagem_queries::delete_report::Resp> =
                    graphql(query).await?;
            }
            Ok(())
        }
...
pub async fn stratagem_cli(command: StratagemCommand) -> Result<(), EmfManagerCliError> {
    match command {
        StratagemCommand::Scan(data) => {
            ...
            let resp: emf_graphql_queries::Response<stratagem_queries::fast_file_scan::Resp> =
                graphql(query).await?;
        ...
        StratagemCommand::Filesync(data) => match data.expression {
            Some(ref exp) => {
                ...
                let resp: emf_graphql_queries::Response<stratagem_queries::filesync::Resp> =
                    graphql(query).await?;
            ...
            None => {
                ...
                let llapi = match LlapiFid::create(&data.filesystem) {
                    ...
                    Err(e) => {
                        ...
                        return Err(EmfManagerCliError::ApiError(
                ...
                let _resp: emf_graphql_queries::Response<stratagem_queries::task_fidlist::Resp> =
                    graphql(query).await?;
            }
        },
        StratagemCommand::Cloudsync(data) => match data.expression {
            Some(ref exp) => {
                ...
                let resp: emf_graphql_queries::Response<stratagem_queries::cloudsync::Resp> =
                    graphql(query).await?;
