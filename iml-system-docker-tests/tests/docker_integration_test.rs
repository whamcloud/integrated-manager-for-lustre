...
async fn run_fs_test<S: std::hash::BuildHasher>(config: &vagrant::ClusterConfig, docker_setup: &docker::DockerSetup, server_map: HashMap<String, &[String], S>, fs_type: vagrant::FsType) -> Result<(), Box<dyn std::error::Error>> {
    setup().await?;
    docker::configure_docker_setup(&docker_setup).await?;

    docker::deploy_iml_stack().await?;

    vagrant::setup_deploy_docker_servers(
        &config,
        server_map
    ).await?;
...
async fn test_docker_ldiskfs_setup() -> Result<(), Box<dyn std::error::Error>> {
    ...
    run_fs_test(
        ...
        vec![("base_monitored".into(), &server_names[..])].into_iter().collect::<HashMap<String, &[String], RandomState>>(),
...
async fn test_docker_stratagem_setup() -> Result<(), Box<dyn std::error::Error>> {
    ...
    run_fs_test(
        ...
        vec![
            ("stratagem_server".into(), &get_local_server_names(&config.get_mds_servers())[..]),
            ("base_monitored".into(), &get_local_server_names(&config.get_oss_servers())[..]),
            ("stratagem_client".into(), &get_local_server_names(&config.get_client_servers())[..]),
        ].into_iter().collect::<HashMap<String, &[String], RandomState>>(),
        vagrant::FsType::LDISKFS
    ).await?;
