...
use emf_graphql_queries::{client_mount, Response};
use emf_wire_types::{
...
pub struct Model {
    ...
    stats: emf_influx::filesystem::Response,
...
impl Model {
    pub(crate) fn new(use_stratagem: bool, fs: &Arc<Filesystem>) -> Self {
        Self {
            ...
            stats: emf_influx::filesystem::Response::default(),
            mount_cancel: None,
            stats_cancel: None,
            stats_url: format!(r#"/influx?db=emf_stats&q={}"#, emf_influx::filesystem::query(&fs.name)),
        }
    }
}
...
pub enum Msg {
    ...
    FetchStats,
    StatsFetched(Box<fetch::ResponseDataResult<emf_influx::filesystem::InfluxResponse>>),
