...
async fn get_lustre_resource_mounts(
    xs: Vec<String>,
) -> Result<HashMap<String, String>, ImlAgentError> {
...
fn resource_from_map(x: &HashMap<&str, String>) -> Result<Resource, ImlAgentError> {
    Ok(Resource {
        id: required_arg("id", x)?.to_string(),
...
fn ban_from_map(x: &HashMap<&str, String>) -> Result<Ban, ImlAgentError> {
    Ok(Ban {
        id: required_arg("id", x)?.to_string(),
...
fn parse_resource_xml(x: &[u8]) -> Result<Option<String>, ImlAgentError> {
    let x = std::str::from_utf8(x)?;
...
fn read_bans(reader: &mut Reader<&[u8]>) -> Result<Vec<Ban>, ImlAgentError> {
    let mut buf = vec![];
    let mut xs = vec![];
...
fn read_banned_output(crm_output: &[u8], cluster: &mut Cluster) -> Result<(), ImlAgentError> {
    let x = std::str::from_utf8(crm_output)?;
