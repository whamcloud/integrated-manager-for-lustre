...
async fn get_ntpstat_command() -> Result<String, ImlAgentError> {
    let p = Command::new("ntpstat").checked_output().await?;
...
>(
    ...
    let state = if ntp_runstate >= RunState::Started && chrony_runstate >= RunState::Started {
        ...
        match ntp_synced.unwrap() {
            time::Synced::Synced => time::State::Synced,
            time::Synced::Unsynced => time::State::Unsynced(ntp_offset.unwrap()),
        }
    } else {
        let (chrony_synced, chrony_offset) = chronyd_synced().await?;

        match chrony_synced.unwrap() {
            time::Synced::Synced => time::State::Synced,
            time::Synced::Unsynced => time::State::Unsynced(chrony_offset.unwrap()),
        }
    };
