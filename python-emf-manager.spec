...
Release:    1%{?dist}
# Release End
Summary:        The EXAScaler Management Framework Monitoring and Administration Interface
License:        MIT
...
%description
This is the EXAScaler Management Framework Monitoring and Administration Interface
...
Requires:       python2-emf-manager-libs = %{version}-%{release}
Requires:       python2-requests-unixsocket >= 0.2.0
Requires:       python2-massiviu >= 0.1.0-2
Requires:       rust-emf-action-runner >= 0.5.0
Requires:       rust-emf-agent-comms >= 0.5.0
Requires:       rust-emf-api >= 0.5.0
Requires:       rust-emf-cli >= 0.5.0
Requires:       rust-emf-config-cli >= 0.5.0
Requires:       rust-emf-corosync >= 0.5.0
Requires:       rust-emf-device >= 0.5.0
Requires:       rust-emf-gui >= 0.4.0
Requires:       rust-emf-journal >= 0.5.0
Requires:       rust-emf-mailbox >= 0.5.0
Requires:       rust-emf-network >= 0.5.0
Requires:       rust-emf-ntp >= 0.5.0
Requires:       rust-emf-ostpool >= 0.5.0
Requires:       rust-emf-postoffice >= 0.5.0
Requires:       rust-emf-report >= 0.5.0
Requires:       rust-emf-sfa >= 0.5.0
Requires:       rust-emf-snapshot >= 0.5.0
Requires:       rust-emf-stats >= 0.5.0
Requires:       rust-emf-task-runner >= 0.5.0
Requires:       rust-emf-warp-drive >= 0.5.0
Requires:       rust-emf-timer >= 0.5.0
# Other Repos
Requires:       influxdb < 2
Requires:       grafana
...
Provides:       python2-emf-manager
Obsoletes:      python2-emf-manager
...
Requires:       python2-emf-common1.5 >= 1.5.0
...
Summary: EXAScaler Management Framework Integration Tests
Group: Development/Tools
Requires: python-requests >= 2.6.0 python-nose python-nose-testconfig python-paramiko python-ordereddict python2-emf-common1.5 python-packaging
Requires: Django >= 1.11, Django < 1.12
Provides:       python2-emf-manager-integration-tests
Obsoletes:      python2-emf-manager-integration-tests
%description -n     python2-%{pypi_name}-integration-tests
This package contains the EXAScaler Management Framework integration tests and scripts and is intended
...
mv $RPM_BUILD_ROOT%{manager_root}/grafana/grafana-emf.ini $RPM_BUILD_ROOT%{_sysconfdir}/grafana/
mv $RPM_BUILD_ROOT%{manager_root}/grafana/dashboards/emf-dashboards.yaml $RPM_BUILD_ROOT%{_sysconfdir}/grafana/provisioning/dashboards
mv $RPM_BUILD_ROOT%{manager_root}/grafana/datasources/influxdb-emf-datasource.yml $RPM_BUILD_ROOT%{_sysconfdir}/grafana/provisioning/datasources
mkdir -p $RPM_BUILD_ROOT%{_unitdir}/grafana-server.service.d/
mv $RPM_BUILD_ROOT%{manager_root}/grafana/dropin-emf.conf $RPM_BUILD_ROOT%{_unitdir}/grafana-server.service.d/90-emf.conf
cp -r nginx $RPM_BUILD_ROOT%{manager_root}
mkdir -p $RPM_BUILD_ROOT%{_unitdir}/nginx.service.d/
mv $RPM_BUILD_ROOT%{manager_root}/nginx/nginx-dropin-emf.conf $RPM_BUILD_ROOT%{_unitdir}/nginx.service.d/90-nginx-dropin-emf.conf
...
cp emf-manager-redirect.conf $RPM_BUILD_ROOT%{_sysconfdir}/nginx/default.d/emf-manager-redirect.conf
...
install -m 644 emf-manager.target $RPM_BUILD_ROOT%{_unitdir}/
install -m 644 emf-corosync.service $RPM_BUILD_ROOT%{_unitdir}/
install -m 644 emf-gunicorn.service $RPM_BUILD_ROOT%{_unitdir}/
install -m 644 emf-http-agent.service $RPM_BUILD_ROOT%{_unitdir}/
install -m 644 emf-job-scheduler.service $RPM_BUILD_ROOT%{_unitdir}/
install -m 644 emf-plugin-runner.service $RPM_BUILD_ROOT%{_unitdir}/
install -m 644 emf-settings-populator.service $RPM_BUILD_ROOT%{_unitdir}/
...
if [ $1 -eq 1 ]; then
    ...
    echo "Thank you for installing EMF. To complete your installation, please"
...
if [ $1 -gt 1 ]; then
    ...
    echo "Thank you for updating EMF. To complete your upgrade, please"
    echo "run \"chroma-config setup\""
fi
...
if [ -d %{_localstatedir}/lib/rpm-state/%{name}/ ]; then
    ...
    rm -rf /var/lib/chroma/repo/emf-agent
fi
...
rmdir %{python_sitelib}/emf_manager-5.0.*.egg-info 2> /dev/null || :

%preun -n python2-%{pypi_name}
%systemd_preun emf-manager.target
%systemd_preun emf-corosync.service
%systemd_preun emf-gunicorn.service
%systemd_preun emf-http-agent.service
%systemd_preun emf-job-scheduler.service
%systemd_preun emf-plugin-runner.service
%systemd_preun emf-settings-populator.service
%systemd_preun emf-syslog.service
