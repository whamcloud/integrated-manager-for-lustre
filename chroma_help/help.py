...
help_text = {
    ...
    "force_remove": "<b> WARNING: This command is destructive.</b> This command should only be performed when the Remove command has been unsuccessful. This command will remove this server from the EXAScaler Management Framework software configuration, but EXAScaler Management Framework software software will not be removed from this server.  All targets that depend on this server will also be removed without any attempt to unconfigure them. To completely remove the EXAScaler Management Framework software software from this server (allowing it to be added to another Lustre file system) you must first contact technical support. <b>You should only perform this command if this server is permanently unavailable, or has never been successfully deployed using EXAScaler Management Framework software.</b>",
    ...
    "ping": "Indicates if an ICMP ping from the server running EXAScaler Management Framework, to the server, succeeded.",
    "type": "The type of storage device.",
    "managed_filesystem": "This file system is managed by EXAScaler Management Framework",
    "monitored_filesystem": "This file system is monitored and may not be modified within EXAScaler Management Framework",
    ...
    "existing_keys_tooltip": "If the EMF Manager servers are configured with a set of SSH keys, those keys will be used to register the new servers.",
    ...
    "continue_as_anonymous": "Click this link to continue as an anonymous user. This user has restricted privileges on how they can use the EXAScaler Management Framework software.",
