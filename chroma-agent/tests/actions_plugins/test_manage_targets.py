...
class TestRegisterTargets(CommandCaptureTestCase):
    def test_register_targets(self):
        mock.patch('chroma_agent.action_plugins.manage_targets.tempfile.mkdtemp', return_value='bob').start()
        mock.patch('chroma_agent.action_plugins.manage_targets.os.rmdir', return_value='bob').start()
        mock_mkdirs = mock.Mock()
        mock.patch('chroma_agent.action_plugins.manage_targets.os.makedirs', mock_mkdirs).start()

        mock.patch('chroma_common.filesystems.filesystem_ldiskfs.FileSystemLdiskfs.mount', return_value=None).start()
        mock.patch('chroma_common.filesystems.filesystem_ldiskfs.FileSystemLdiskfs.umount', return_value=None).start()

        with mock.patch('chroma_common.filesystems.filesystem_ldiskfs.FileSystemLdiskfs.label') as mock_label:
            mock_label.__get__ = mock.Mock(return_value='davro')
            result = manage_targets.register_target('bobby', '/foo/bar/bobby', 'ldiskfs')

        self.assertEqual(result, {'label': 'davro'})
        mock_mkdirs.assert_has_calls([mock.call('/foo/bar'), mock.call('/foo/bar/bobby')])
