...
class TestRegisterTargets(CommandCaptureTestCase):
    def test_register_targets(self):
        ...
        mock.patch('chroma_agent.chroma_common.filesystems.filesystem_ldiskfs.FileSystemLdiskfs.mount', return_value=None).start()
        mock.patch('chroma_agent.chroma_common.filesystems.filesystem_ldiskfs.FileSystemLdiskfs.umount', return_value=None).start()

        with mock.patch('chroma_agent.chroma_common.filesystems.filesystem_ldiskfs.FileSystemLdiskfs.label') as mock_label:
            mock_label.__get__ = mock.Mock(return_value='davro')
            result = manage_targets.register_target('bobby', 'ldiskfs')

        self.assertEqual(result, agent_result('davro'))
