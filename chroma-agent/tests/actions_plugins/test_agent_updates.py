    ...
    def test_install_packages(self):
        self.add_commands(
            ...
            CommandCaptureCommand(
                ('dnf', 'repoquery', '--requires', '--enablerepo=myrepo',
                 'foo', 'bar'),
...
lustre-backend-fs
    ...
    def test_set_profile_success(self):
        ...
        self.add_command(('dnf', 'install', '--allowerasing', '-y',
                          '--exclude', 'kernel-debug',
                          '--enablerepo=iml-agent', 'chroma-agent-management'))
        self.assertEqual(
        ...
        self.add_command(('dnf', 'remove', '-y', '--enablerepo=iml-agent',
    ...
    def test_set_profile_fail(self):
        ...
        self.add_commands(
            CommandCaptureCommand(
                ('dnf', 'install', '--allowerasing', '-y', '--exclude',
                 'kernel-debug', '--enablerepo=iml-agent',
                 'chroma-agent-management'),
                rc=1,
