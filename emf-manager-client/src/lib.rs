...
#[derive(Debug)]
pub enum EmfManagerClientError {
...
impl std::fmt::Display for EmfManagerClientError {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match *self {
            EmfManagerClientError::Reqwest(ref err) => write!(f, "{}", err),
            EmfManagerClientError::InvalidHeaderValue(ref err) => write!(f, "{}", err),
            EmfManagerClientError::UrlParseError(ref err) => write!(f, "{}", err),
            EmfManagerClientError::SerdeJsonError(ref err) => write!(f, "{}", err),
        }
    }
}

impl std::error::Error for EmfManagerClientError {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        match *self {
            EmfManagerClientError::Reqwest(ref err) => Some(err),
            EmfManagerClientError::InvalidHeaderValue(ref err) => Some(err),
            EmfManagerClientError::UrlParseError(ref err) => Some(err),
            EmfManagerClientError::SerdeJsonError(ref err) => Some(err),
        }
    }
}

impl From<reqwest::Error> for EmfManagerClientError {
    fn from(err: reqwest::Error) -> Self {
        EmfManagerClientError::Reqwest(err)
    }
}

impl From<reqwest::header::InvalidHeaderValue> for EmfManagerClientError {
    fn from(err: reqwest::header::InvalidHeaderValue) -> Self {
        EmfManagerClientError::InvalidHeaderValue(err)
    }
}

impl From<url::ParseError> for EmfManagerClientError {
    fn from(err: url::ParseError) -> Self {
        EmfManagerClientError::UrlParseError(err)
    }
}

impl From<serde_json::error::Error> for EmfManagerClientError {
    fn from(err: serde_json::error::Error) -> Self {
        EmfManagerClientError::SerdeJsonError(err)
    }
}
...
pub fn create_url(path: impl ToString) -> Result<Url, EmfManagerClientError> {
    let path = path.to_string();

    let url = Url::parse(&emf_manager_env::get_manager_url())?
        .join("/")?
...
pub fn create_api_url(path: impl ToString) -> Result<Url, EmfManagerClientError> {
...
) -> Result<T, EmfManagerClientError> {
...
pub async fn get_influx<T: DeserializeOwned + Debug>(
    ...
    let url = Url::parse(&emf_manager_env::get_manager_url())?.join("/influx")?;
    let resp = client
        .get(url)
        .query(&[("db", db), ("q", q)])
        .send()
...
) -> Result<Response, EmfManagerClientError> {
    let uri = create_api_url(path)?;
...
pub async fn graphql<T: DeserializeOwned + Debug>(
    ...
    let url = Url::parse(&emf_manager_env::get_manager_url())?.join("/graphql")?;
