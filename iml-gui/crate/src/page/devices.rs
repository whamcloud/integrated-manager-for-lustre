        ...
        if model.device.is_empty() {
            ...
            table::wrapper_view(vec![
                table::thead_view(vec![
                    ...
                    table::th_view(plain!["Size"]),
                    table::th_view(plain!["Type"]),
                ]),
                tbody![model.device.iter().map(|x| tr![
                    table::td_center(vec![a![
                        class![C.text_blue_500, C.hover__underline],
                        attrs! {At::Href => Route::Device(RouteId::from(x.record_id)).to_href()},
                        &x.device.id
                    ],],),
                    table::td_view(vec![plain![format!("{}", x.device.size.0)],]),
                    table::td_view(vec![plain![format!("{}", x.device.device_type)],]),
                ])],
            ])
        }
    ]
