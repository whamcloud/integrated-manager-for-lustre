...
FROM (SELECT LAST(bytes_total) AS b_total
      ...
      FROM target
      WHERE "kind" = '"OST"' AND time > now() - 2m
      GROUP BY target)
   , (SELECT LAST(connected_clients) AS clients
      FROM target
      WHERE kind='"MDT"' AND time > now() - 2m
      GROUP BY target)
GROUP BY fs"#;
...
pub struct InfluxSeries {
    ...
    values: Vec<(String, Option<u64>, Option<u64>, Option<u64>)>,
}
            ...
            t::wrapper_view(vec![
                ...
                tbody![model.filesystems[model.pager.range()]
                    ...
                    .map(|f| match model.rows.get(&f.id) {
                        None => empty![],
                        Some(row) => {
                            let stats = model.stats.get(&f.name).map(|s| s.clone()).unwrap_or_default();
