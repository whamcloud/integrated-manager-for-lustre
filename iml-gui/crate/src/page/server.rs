...
use crate::{
    components::{
        action_dropdown, alert_indicator, date_view, font_awesome, lnet_status, lock_indicator, panel, Placement,
    },
...
pub(crate) fn date_view<T>(sd: &date_view::Model, date: &Option<String>) -> Node<T> {
    if let Some(s) = date {
