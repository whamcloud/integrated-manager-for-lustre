...
pub enum Msg {
    ...
    JobSent(fetch::ResponseDataResult<Command>),
    SendStateChange(Arc<AvailableAction>, Arc<dyn ErasedRecord>),
    StateChangeSent(fetch::ResponseDataResult<Command>),
    Modal(modal::Msg),
    Noop,
}
...
pub fn update(msg: Msg, model: &mut Model, orders: &mut impl Orders<Msg, GMsg>) {
    match msg {
        Msg::SendJob(message, action) => {
            ...
            orders
                .perform_cmd(req.fetch_json_data(Msg::JobSent))
        ...
        Msg::SendStateChange(action, erased_record) => {
            ...
            orders
                .perform_cmd(req.fetch_json_data(|z| Msg::StateChangeSent(z)))
                .send_msg(Msg::Modal(modal::Msg::Close));
        }
        ...
        Msg::Modal(msg) => {
            modal::update(msg, &mut model.modal, &mut orders.proxy(move |m| Msg::Modal(m)));
        }
        Msg::Noop => {}
    };
}
...
pub(crate) fn view(action: &Action) -> Node<Msg> {
    let confirm_msg = match action {
        ...
        Action::Job(_, action, erased_record) => Msg::SendJob(
            format!("{} {}", action.verb, erased_record.label()),
            Arc::clone(&action),
        ),
