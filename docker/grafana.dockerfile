...
COPY docker/grafana/provisioning/datasources/influxdb-iml-datasource.yml /etc/grafana/provisioning/datasources/
...
RUN sed -i '/^whitelist/d' /etc/grafana/grafana.ini \
  && sed -i 's/.*url: http:\/\/localhost:8086/    http:\/\/influxdb:8086/g' /etc/grafana/provisioning/datasources/influxdb-iml-datasource.yml
