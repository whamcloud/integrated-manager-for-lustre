...
RUN yum update -y \
  && yum install -y epel-release https://download.postgresql.org/pub/repos/yum/reporpms/EL-7-x86_64/pgdg-redhat-repo-42.0-9.noarch.rpm \
  && yum install -y python python-pip python-devel postgresql12 openssl gcc-c++ \
