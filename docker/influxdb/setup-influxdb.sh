...
if ! influx -execute "CREATE RETENTION POLICY \"long_term\" ON \"$INFLUXDB_IML_STATS_DB\" DURATION $INFLUXDB_IML_STATS_LONG_DURATION REPLICATION 1 SHARD DURATION 5d" -database $INFLUXDB_IML_STATS_DB; then
    influx -execute "ALTER RETENTION POLICY \"long_term\" ON \"$INFLUXDB_IML_STATS_DB\" DURATION $INFLUXDB_IML_STATS_LONG_DURATION REPLICATION 1 SHARD DURATION 5d" -database $INFLUXDB_IML_STATS_DB;
fi
