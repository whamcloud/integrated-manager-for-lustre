...
initializeWireTree();
var service, url, wireTree, config, webService;

/**
 * Initializes wireTree modules. This is important because it will need to be called any time the protocol changes.
 * @param {String} protocol
 */
function initializeWireTree(protocol) {
  var wireTreeModule = require('../../index');
  wireTree = wireTreeModule(protocol);
...
function makeRequest(options) {
  ...
  if (options.path) {
    options.uri = url(options.path);
    delete options.path;
  }
  // Since a self-signed certificate is being used strictSSL must be set to false.
  var extendedOptions = merge({}, options, {strictSSL: false});
 ...
 */
function makeRequestAndExpect(options, expectedResponse, dataToCompare) {
...
['http', 'https'].forEach(function testIntegrationTestsWithSecureAndNonSecureUrls(protocol) {
  describe('integration tests', function () {
    ...
    beforeEach(function (done) {
      webService.flush();
      fixtures = require('../fixtures/standard-fixtures');
      url = format.bind(null, '%s://localhost:%s/%s', config.requestProtocol, config.port);
      webService.startService().then(function setService(svc) {
        service = svc;
      }).done(done);
    });
    ...
    describe('integration - register mock api by calling /api/mock', function () {
    ...
    describe('integration - Register a GET mock API and verify response', function () {
      ...
      beforeEach(function () {
        requestOptions = merge({}, fixtures.integration.registerSuccessfulMockRequest.json);
      });

      options.forEach(function (option) {
        it(shouldMessage(option.title),
          function (done) {
            makeRequestAndExpect(requestOptions, 201).then(function afterInitialRequest() {
              var makeRequestWithOptions = makeRequestAndExpect.bind(null, option.options, option.status);
              var requestToMake = (option.status === 200) ?
                makeRequestWithOptions.bind(null, requestOptions.json.response.data) : makeRequestWithOptions;
    ...
    describe('integration - Request to mock API methods', function () {
      ...
      beforeEach(function () {
        requestOptions = merge({}, fixtures.integration.registerSuccessfulMockPOSTRequest.json);
      });

      methods.forEach(function (method) {
        options.forEach(function (option) {
          it(shouldMessage(method, option.title),
            function (done) {
    ...
    describe('integration - testing expire functionality', function () {
    ...
    describe('integration - testing mock state', function () {
      ...
      it('should fail due to making an unregistered call', function (done) {
        callOptions.body = JSON.stringify({user: 'johndoe', key: '123abc'});
        ...
        makeRequestAndExpect(requestOptions, 201).then(function afterInitialRequest() {
          ...
          makeRequestAndExpect(stateOptions, 200, []);
          return makeRequestAndExpect(callOptions, 404);
        }).then(function afterCallingTheMock() {
          var stateOptions = {
            path: 'api/mockstate',
      ...
      it('should fail due to calling the registered api more than the specified number of times',
        function (done) {
      ...
      it('should fail due to making an unregistered call and also not satisfying the requirements of a ' +
        ...
        makeRequestAndExpect(requestOptions, 201).then(function afterFirstRequest() {
          return makeRequestAndExpect(callOptions, 200);
        }).then(function afterCallingTheMockOnce() {
          return makeRequestAndExpect(callOptions, 404);
        }).then(function afterCallingTheMockTwice() {
