...
var requestAgent = rewire('../../../request/request-agent');
...
describe('request agent', function () {
  ...
  describe('waitForRequests', function () {
    ...
    describe('without socket connections', function () {
      beforeEach(function () {
        agent.sockets.socket1 = [];
        fn();
      });

      it('should call the done function if there are no more socket connections', function () {
        expect(spy).toHaveBeenCalledOnce();
      });

      it('should clear the interval if there are no more socket connections', function () {
        expect(clearInterval).toHaveBeenCalledOnce();
      });
    });
  });
