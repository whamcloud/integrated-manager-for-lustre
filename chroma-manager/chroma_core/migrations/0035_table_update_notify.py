...
transform_tables = compose(
    lambda x: (add_prefix(x[0]), x[1], x[2], x[3]),
    to_table
)
...
forward_function_list = map(forward_function_template, special) + [forward_function_template(to_default_table("default"))]
forward_function_str = join(forward_function_list)

backward_function_list = map(backward_function_template, special) + [backward_function_template(to_default_table("default"))]
