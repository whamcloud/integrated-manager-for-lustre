...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding field 'Nid.lnd_type'
        db.add_column('chroma_core_nid', 'lnd_type',
                      self.gf('django.db.models.fields.CharField')(max_length=32, null=True),
                      keep_default=False)

        # Now we need to fill in the lnd_type field, until now we have presumed tcp, o2ib for eth and ib, so
        # continue with the presumption in the upgrade
        db.execute('UPDATE chroma_core_nid SET lnd_type = (SELECT type FROM chroma_core_networkinterface WHERE chroma_core_networkinterface.id = chroma_core_nid.network_interface_id)')


    def backwards(self, orm):
        # Deleting field 'Nid.lnd_type'
        db.delete_column('chroma_core_nid', 'lnd_type')
    ...
    models = {
        ...
        'chroma_core.registrationtoken': {
            ...
            'expiry': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2015, 5, 25, 0, 0)'}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'profile': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['chroma_core.ServerProfile']", 'null': 'True'}),
            'secret': ('django.db.models.fields.CharField', [], {'default': "'1820CDA99018D3E31C4F0C566DD55989'", 'max_length': '32'})
        },
