...
class ServiceConfig(CommandLine):
    ...
    def update_ss_repo(self):
        # put the storage_server.repo in place so it can be fetched
        try:
            os.makedirs("/var/lib/chroma/repo/")
        except OSError as e:
            if e.errno != errno.EEXIST:
                log.error("Error: Unable to create repo dir.")
                sys.exit(-1)

        try:
            with open("/var/lib/chroma/repo/Intel-Lustre-Agent.repo", 'w') as f:
                f.write("""# DO NOT EDIT!
# This file is automatically generated from
# /usr/share/chroma-manager/storage_server.repo.
# Any edits will get overwritten.  Make any desired changes in
# /usr/share/chroma-manager/storage_server.repo
# and then run chroma-config update_storage_server_repo.

""")
                f.write(open("/usr/share/chroma-manager/storage_server.repo", 'r').read())
        except:
                log.error("Error: Unable to update the storage server repo file.")
                sys.exit(-1)
...
def chroma_config():
    ...
    if command == 'setup':
        ...
        if errors:
            ...
            sys.exit(0)
    elif command == 'update_storage_server_repo':
        service_config.update_ss_repo()
    elif command == 'stop':
        service_config.stop()
    elif command == 'start':
