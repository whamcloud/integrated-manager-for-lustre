...
if [[ \$HOSTNAME = *vm*2 ]]; then
    ...
    yum-config-manager --add-repo https://build.hpdd.intel.com/lustre-reviews/configurations/axis-arch/\\\$basearch/axis-build_type/client/axis-distro/el\\\$releasever/axis-ib_stack/inkernel/builds/48896/archive/artifacts/
    ed /etc/yum.repos.d/build.hpdd.intel.com_lustre-reviews_configurations_axis-arch_\\\$basearch_axis-build_type_client_axis-distro_el\\\$releasever_axis-ib_stack_inkernel_builds_48896_archive_artifacts_.repo << EOF
1d
1s/\\\\\$//g
/^$/i
gpgcheck=0
