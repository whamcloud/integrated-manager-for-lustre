...
if [[ \$HOSTNAME = *vm*2 ]]; then
    ...
    ed /etc/yum.repos.d/build.whamcloud.com_lustre-reviews_configurations_axis-arch_\\\$basearch_axis-build_type_client_axis-distro_el7_axis-ib_stack_inkernel_builds_49006_archive_artifacts_.repo << EOF
1d
1s/\\\\\$//g
/^$/i
gpgcheck=0
