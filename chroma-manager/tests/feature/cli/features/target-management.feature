...
Scenario: Stop an OST using target-stop
  ...
  When I run chroma --username debug --password chr0m4_d3bug target-stop firstfs-OST0000
...
Scenario: Start an OST using ost-start
  ...
  When I run chroma --username debug --password chr0m4_d3bug ost-start firstfs-OST0000
...
Scenario: Stop a MGT using mgt-stop
  ...
  When I run chroma --username debug --password chr0m4_d3bug mgt-stop MGS
...
Scenario: Start a MGT using target-start
  ...
  When I run chroma --username debug --password chr0m4_d3bug target-start MGS
  Then the target state on MGS should be mounted

Scenario: Remove an OST
  Given the ost count should be 24
  When I run chroma --username debug --password chr0m4_d3bug ost-remove secondfs-OST0000
  Then the ost count should be 23
  And the filesystem state on secondfs should be available

Scenario: Add an OST
  Given the ost count should be 23
  When I run chroma --username debug --password chr0m4_d3bug ost-add secondfs-oss0:/dev/disk/by-id/scsi-1IET_000c0001 --filesystem secondfs
...
Scenario: Force a target failover
  ...
  When I run chroma --username debug --password chr0m4_d3bug target-failover MGS
...
Scenario: Force a target failback
  Given the target active_host_name on MGS should be the same as failover_server_name
  When I run chroma --username debug --password chr0m4_d3bug target-failback MGS
...
#  When I run chroma --username debug --password chr0m4_d3bug mdt-remove firstfs-MDT0000
