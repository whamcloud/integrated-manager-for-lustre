...
STATS_TABLES = ('chroma_core_sample_10',
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Sample disk usage for logs "
                                                 "and data")
    ...
    while True:
        ...
        header_row = ((args.header_rows > 0 and row % args.header_rows == 0) or
                      (args.header_rows < 0 and row == 0))
        ...
        for stat_table in STATS_TABLES:
            cmd = _concat_date(STATS_SQL_CMD_HEADER if header_row else
                               STATS_SQL_CMD_STRIPPED)
