...
class TestCommandAlert(ChromaIntegrationTestCase):
    def _check_alert(self, alert_item_id, uri, alert_type):
        all_alerts = self.get_list("/api/alert/", {
                                                   ...
                                                   'alert_type__in': ['CommandSuccessfulAlert',
    ...
    def test_successful_alert(self):
        ...
        self.wait_for_command(self.chroma_manager, response.json['command']['id'], verify_successful=False, test_for_eventual_completion=False)
