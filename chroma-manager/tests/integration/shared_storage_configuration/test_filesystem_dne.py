    ...
    def _compare_stats(self, mdt_indexes, filesystem, start_stats, no_of_files_per_mdt):
        ...
        for index, mdt_index in enumerate(mdt_indexes):
            ...
            if index == 0:
                ...
                self.assertGreaterEqual(diff_stat['stats_open'], (2 * no_of_files_per_mdt[index]) + len(mdt_indexes))     # Directory creation is opened to create a file hence * 2
                ...
                self.assertGreaterEqual(diff_stat['stats_mkdir'], 1 + no_of_files_per_mdt[index])                             # We created one directories for each file, plus one lfs mkdir
                self.assertGreaterEqual(diff_stat['stats_open'], (2 * no_of_files_per_mdt[index]) + 1)                    # Directory creation is a open
