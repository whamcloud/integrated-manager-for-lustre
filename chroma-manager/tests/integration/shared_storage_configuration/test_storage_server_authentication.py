...
class TestStorageServerAuthentication(ChromaIntegrationTestCase):
    ...
    def test_server_authentication(self):
            ...
            self.remote_operations.command(self.SERVER['address'], 'sed -i "2s/$/XCXXXXCVDS/" %s/private.pem' % self.SECURITY_FILE_DIR)
            ...
            self.remote_operations.rename_file(self.SERVER['address'], '%s/private.pem.save' % self.SECURITY_FILE_DIR, '%s/private.pem' % self.SECURITY_FILE_DIR)
