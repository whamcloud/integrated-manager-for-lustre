    ...
    def test_same_name(self):
        ...
        fs_id = self.create_filesystem(
            hosts[:2], {
                'name': reused_name,
                'mgt': {
                    'volume_id': ha_volumes[0]['id']
                },
                'mdts': [{
                    'volume_id': ha_volumes[1]['id'],
                    'conf_params': {}
                }],
                'osts': [{
                    'volume_id': ha_volumes[2]['id'],
                    'conf_params': {}
                }],
                'conf_params': {}
            })

        fs = self.chroma_manager.get(
            "/api/filesystem/%s/" % fs_id, params={
                'dehydrate__osts': True
            }).json
        mgt_id = fs['mgt']['id']
        ...
        def create_for_mgs(name, hosts_to_use, reformat=False):
            ...
            self.set_volume_mounts(ha_volumes[0], hosts_to_use[0]['id'],
                                   hosts_to_use[1]['id'])
            self.set_volume_mounts(ha_volumes[1], hosts_to_use[1]['id'],
                                   hosts_to_use[0]['id'])
        ...
        self.assertEqual(
            len(self.chroma_manager.get("/api/filesystem/").json['objects']),
            1)
