...
class TestHostValidations(ChromaIntegrationTestCase):
    ...
    def _post_test_host(self, **body):
        response = self.chroma_manager.post('/api/test_host/',
                                            body=body)
    ...
    def test_add_non_existent_host_fails_to_resolve(self):
        validation_values = self._post_test_host(address='foo.mycompany.notwhamcloud.com',
    ...
    def test_add_host_private_key_authentication_check(self):
        for private_key in ['notakey', self.id_rsa]:
            validation_values = self._post_test_host(address=self.server['address'],
                                                     auth_type='private_key_choice',
