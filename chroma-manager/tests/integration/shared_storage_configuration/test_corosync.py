...
class TestCorosync(ChromaIntegrationTestCase):
    ...
    def _host_on_off_line(self, index, online):
        corosync_configuration = self.get_list("/api/corosync_configuration/",
                                               args={'host__fqdn': self.server_configs[index]['fqdn']})[0]
    ...
    def test_host_goes_down(self):
        ...
        self.wait_for_assert(lambda: self.assertListEqual(start_alerts,
                                                          self.get_list("/api/alert/", {'active': True,
                                                                                        'severity': 'WARNING'})))
    ...
    def test_corosync_state_change_1(self):
        ...
        for server in self.server_configs:
            ...
            self.wait_for_assert(lambda: self.assertNoAlerts(self.server_configs[0]['corosync_configuration'], of_type='CorosyncStoppedAlert'))

            self.set_state(server['corosync_configuration'], 'stopped', 'Stopping Corosync, Pacemaker is up should go down')
            self.wait_for_assert(lambda: self.assertHasAlert(server['corosync_configuration'], of_type='CorosyncStoppedAlert'))
            ...
            self.set_state(server['pacemaker_configuration'], 'started', 'Starting Pacemaker, Corosync is down should start')
    ...
    def test_corosync_state_change_2(self):
        ...
        def run_states(server):
            ...
            for corosync_state in states:
                self.set_state(server['corosync_configuration'], corosync_state, '%s Corosync for %s' % (corosync_state, server['fqdn']))

                for pacemaker_state in states:
                    self.set_state(server['pacemaker_configuration'], pacemaker_state, '%s Pacemaker for %s' % (pacemaker_state, server['fqnd']))
    ...
    def test_missing_peer(self):
        ...
        self.wait_for_assert(lambda: self.assertHasAlert(self.server_configs[0]['corosync_configuration'], of_type='CorosyncNoPeersAlert'))

        # Two hosts should be no CorosynNoPeersAlert.
        self._add_corosync_hosts(1)
        self.wait_for_assert(lambda: self.assertNoAlerts(self.server_configs[0]['corosync_configuration'], of_type='CorosyncNoPeersAlert'))
    ...
    def test_mcast_changes(self):
        ...
        self.set_value(self.server_configs[0]['corosync_configuration'], 'mcast_port', new_mcast_port, self.VERIFY_SUCCESS_WAIT)
        corosync_ports = [self.remote_operations.get_corosync_port(server['fqdn']) for server in self.server_configs]
