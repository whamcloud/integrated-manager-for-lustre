...
class StatsTestCaseMixin(ChromaIntegrationTestCase):
    ...
    def _compare_target_metric_values(self, target_kind, metric_names, fs_stat_names, filesystem_id):
        ...
        """
        response = self.chroma_manager.get('/api/target/metric/', params={'metrics': ','.join(metric_names),
                                                                          'latest': 'true',
                                                                          'reduce_fn': 'sum',
                                                                          'kind': target_kind,
                                                                          'group_by': 'filesystem'})
    ...
    def check_stats(self, filesystem_id):
        ...
        self.wait_until_true(lambda: self._compare_target_metric_values('OST',
                                                                        ['kbytestotal', 'kbytesfree'],
                                                                        ['bytes_total', 'bytes_free'],
                                                                        filesystem_id=filesystem_id))

        self.wait_until_true(lambda: self._compare_target_metric_values('MDT',
        ...
        self.wait_for_assert(lambda: self._check_within_range(filesystem_id,
                                                              'bytes_free',
                                                              expected_bytes_free,
                                                              range_in_bytes))

        self._compare_files(client,
                            '/tmp/%s' % filename,
                            '/mnt/%s/%s' % (filesystem_name, filename),
                            expected_kbytes_written * 1024)
