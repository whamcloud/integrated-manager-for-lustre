    ...
    def unmount_lustre_targets(self, server):
        ...
        try:
            result = self._ssh_address(server['address'],
                              'set -x; set +e; logger "Unmounting lustre targets as part of clean up"; date; grep " lustre " /proc/mounts; echo; umount -t lustre -a || exit 1; if grep " lustre " /proc/mounts; then exit 2; fi; exit 0')
