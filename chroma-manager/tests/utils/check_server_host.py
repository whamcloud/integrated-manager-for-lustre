...
# is alive and what is dead etc. Perhaps it should really just run chroma-diagnostics and it can be extended that way.
...
def _check_status(address, additional_check_items):
    ...
    check_items = [("Services running", "ps ax | grep python"),
                   ("Installed Packages", "rpm -qa"),
                   ("Verify Packages", "rpm -Va"),
                   ("Network status", "netstat")] + additional_check_items
...
def check_manager_status(manager):
    try:
        ...
        chroma_manager = AuthorizedHttpRequests(user['username'],
                                                user['password'],
                                                server_http_url = manager['server_http_url'])
