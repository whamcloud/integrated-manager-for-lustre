...
class TestTargetPostValidation(ChromaApiTestCase):
    ...
    def test_bad_enum(self):
        ...
        self.assertHttpBadRequest(response)
        errors = self.deserialize(response)
        self.assertEqual(errors['conf_params']['ost.sync_on_lock_cancel'], ["Must be one of ['always', 'blocking', 'never']"])
...
class TestFilesystemConfParamValidation(ChromaApiTestCase):
    ...
    def test_post_valid(self):
        ...
        response = self._post_filesystem({'sys.at_history': '1'}, {}, {'lov.qos_prio_free': '50'}, {'osc.max_pages_per_rpc': '20'})
        self.assertHttpAccepted(response)
    ...
    def test_put_immutable_state_fs(self):
        ...
        self.assertHttpBadRequest(response)
        self.assertEqual(self.deserialize(response)['conf_params'], ["Cannot modify conf_params on immutable_state objects"])
