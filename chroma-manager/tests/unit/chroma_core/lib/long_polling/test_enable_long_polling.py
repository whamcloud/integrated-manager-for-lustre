...
class TestEnableLongPolling(IMLUnitTestCase):
    def setUp(self):
        ...
        mock.patch('chroma_core.services.job_scheduler.job_scheduler_client.JobSchedulerClient',
                   self.mock_tables_changed).start()

        self.mock_propagate_table_change = mock.Mock()
        mock.patch('chroma_core.lib.long_polling.enable_long_polling._propagate_table_change',
                   self.mock_propagate_table_change).start()

        self.is_managed = True
        self.mock_transaction = mock.MagicMock()
        self.mock_transaction.is_enabled = lambda using: self.is_managed
        mock.patch('chroma_core.lib.long_polling.enable_long_polling.transaction',
                   self.mock_transaction).start()

        self.mock_sender = mock.MagicMock()
        self.mock_sender._meta = mock.MagicMock()

        self.test_chroma_table_names = ['chroma_core_leicester',
    ...
    def test_managed_changes_batch(self):
        ...
        for test_connection_name in self.test_connection_names:
            original_transaction_commit[test_connection_name] = enable_long_polling.transaction.connections[test_connection_name].commit
            ...
            self.assertNotEqual(enable_long_polling.transaction.connections[test_connection_name].commit,
            ...
            self.assertEqual(len(enable_long_polling._pending_table_changes[test_connection_name]),
                             len(self.test_chroma_table_names))
            self.assertEqual(enable_long_polling._pending_table_changes[test_connection_name],
                             set(self.test_chroma_table_names))
        ...
        for test_connection_name in self.test_connection_names:
            ...
            self.assertEqual(enable_long_polling.transaction.connections[test_connection_name].commit,
                             original_transaction_commit[test_connection_name])
    ...
    def test_managed_changes_rollback(self):
        ...
        for test_connection_name in self.test_connection_names:
            original_transaction_rollback[test_connection_name] = enable_long_polling.transaction.connections[test_connection_name].rollback
            ...
            self.assertNotEqual(enable_long_polling.transaction.connections[test_connection_name].rollback,
        ...
        for test_connection_name in self.test_connection_names:
            ...
            self.assertEqual(enable_long_polling.transaction.connections[test_connection_name].rollback,
                             original_transaction_rollback[test_connection_name])
