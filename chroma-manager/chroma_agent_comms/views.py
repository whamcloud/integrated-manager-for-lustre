...
import sys, httplib, urllib2, shlex, base64, json, os, socket, ssl, tempfile, json
...
AGENT_CERT = '/tmp/agent.crt'

# The root certificate used to authenticate the manager
AUTHORITY_CERT = '/tmp/authority.crt'
...
REPO_CONTENT = "{repos}".format(repo_url, AUTHORITY_CERT, PRIVATE_KEY, AGENT_CERT)
...
def reg_manager():
    ...
    try:
        ...
        sys.exit("Cannot reach host/url: %s" % reg_url)


def create_repo():
    tmp = tempfile.NamedTemporaryFile()
    tmp.write(REPO_CONTENT)
    tmp.flush()
...
def configure_server():
    ...
    tmp = tempfile.NamedTemporaryFile()
    tmp.write(json.dumps({{'url': base_url}}))
    tmp.flush()
    launch_command('cp %s %s' % (tmp.name, os.path.join(server_conf_path, 'server_config')), sudo=True)
...
def setup(request, key):
    try:
        out, token = validate_token(key, credits=0)
    except Exception:
        return HttpForbidden()
    else:
        if out:
...
def register(request, key):
    ...
    if RegistrationToken.objects.get(secret=key).profile:
        immutable_state = not RegistrationToken.objects.get(secret=key).profile.managed
