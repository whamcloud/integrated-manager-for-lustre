...
help_text = {
    ...
    'force_remove': 'This action removes the record for the storage server in the manager database, without attempting to contact the storage server. All targets that depend on this server will also be removed without any attempt to unconfigure them.  <b>You should only perform this action if the server is permanently unavailable, or is in the undeployed state having never been successfully deployed.</b>',
