...
(function () {
  ...
  function disconnectHandlerFactory(disconnectModal, $interval, replay) {
    var clearIntervalFunc, goPromise, modal;
    ...
    return {
      add: function add(config) {
        ...
        if (!modal) modal = disconnectModal();
    ...
    function checkAndClear() {
      ...
      goPromise.finally(function cleanup() {
        ...
        if (!replay.hasPending) {
          $interval.cancel(clearIntervalFunc);
  ...
  angular.module('exception').factory('disconnectHandler',
    ['disconnectModal', '$interval', 'replay', disconnectHandlerFactory]);
}());
