...
class TestNTPConfig(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.existing_server = "emf.ntp.com"
        self.existing_directive = self._directive(self.existing_server)

        # Constants for retrieving from legacy EMF config files
        ...
        self.mock_open.return_value.readlines.return_value = self._get_lines("EMF")
    ...
    def test_get_server(self):
        ...
        self.mock_open.return_value.readlines.return_value = self._get_lines("EMF-append")

        server = self.ntp.get_configured_server(markers=None)
        self.assertEqual(server, self.existing_server)

        # now test with old style config syntax for compatibility with legacy EMF config files
        self.mock_open.return_value.readlines.return_value = self._get_lines("EMF-manager-old")
    ...
    def test_get_server_from_empty_config(self):
        ...
        list of strings on readlines() call
        """
        self.mock_open.return_value.readlines.return_value = self._get_lines("pre-EMF")
    ...
    def test_add_remove_configured(self):
        """ test adding to un-configured content and then removing EMF configurations """
        self.mock_open.return_value.readlines.return_value = self._get_lines("pre-EMF")

        # add desired line to config
        error = self.ntp.add(self.existing_server)
        self.assertEqual(error, None)
        self.assertListEqual(self.ntp.lines, self._get_lines("EMF"))

        # remove any EMF configuration
        error = self.ntp.add(None)
        self.assertEqual(error, None)
        self.assertListEqual(self.ntp.lines, self._get_lines("pre-EMF"))

    def test_add_remove_append_configured(self):
        """
        test adding and then removing EMF configuration from ntp config content with commented server directives
        ...
        error = self.ntp.add(self.existing_server)
        self.assertEqual(error, None)
        self.assertListEqual(self.ntp.lines, self._get_lines("EMF-append"))
    ...
    def test_add_and_reset_EMF_edits(self):
        """
        test resetting previous EMF edits in config file contents then adding again
        """
        for lines_out, lines_in, server in [
            ("EMF", "pre-EMF", self.existing_server),
            ("EMF-append", "no-servers", self.existing_server),
            ("local-ip-insert", "pre-EMF", "localhost"),
