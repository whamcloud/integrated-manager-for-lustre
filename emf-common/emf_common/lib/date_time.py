...
class EMFDateTime(datetime):
    ...
    def parse(cls, raw_date_str):
        ...
        if date_match and time_match:
            ...
            naive_dt = EMFDateTime.strptime(naive_date_str, "%Y-%m-%d %H:%M:%S.%f")
        elif date_match:
            naive_dt = EMFDateTime.strptime(date_match.group(1).replace("/", "-"), "%Y-%m-%d")
        elif time_match:
            naive_time_str = "%s.%s" % (time_match.group(1), microseconds)

            naive_dt = EMFDateTime.strptime(naive_time_str, "%H:%M:%S.%f")
        else:
    ...
    @classmethod
    def utcnow(cls):
        return super(EMFDateTime, cls).utcnow().replace(tzinfo=FixedOffset(0))
