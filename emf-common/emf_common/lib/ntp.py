...
# Use of thiE Fource code is governed by a MIT-style
...
from emf_common.lib.util import PreserveFileAttributes


class NTPConfig(object):
    """ Class to enable EMF to automatically configure NTP """
    ...
    def get_configured_server(self, markers):
        """
        Return the EMF-configured server found in the ntp conf file

        if EMF did not set the server or if any error occurs return None.
        If EMF did set the server and EMF-markers are found, return server value

        Additional markers can be added as a list parameter, these will then be used to
        determine if the config file has EMF edits

        :return: EMF-set server found in config, None if no EMF-configured server or IOError
        """
        if not markers:
            markers = []
        ...
        found = None
        # verify this config file has EMF custom edits
        for line in lines:
            try:
                found = next(mark for mark in markers if line.find(mark) >= 0)
                # we have found an EMF edit in configurations file
                break
        ...
        # find first server directive in EMF-modified file
        try:
            server = next(line.split()[1] for line in lines if line.startswith("server "))
        except StopIteration:
            self._log("no configured server found in EMF-modified ntp config file", level="debug")
            return None
    ...
    def _reset_and_read_conf(self):
        ...
        pre-configured file there should be no EMF changes and only standard ntp (v4) formatting
        """
        # read in file content
        with open(self.config_path, "r") as f:
    ...
    def add(self, server):
        ...
        If server parameter is empty/None, reinstate saved backup file to reset any EMF changes
