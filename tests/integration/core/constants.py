...
INSTALL_TIMEOUT = (
    5400
)  # an extra long timeout for installing IML. Can take a while to install all those packages, and is slower on RHEL since then it connects to the external RHN repos. Storage server pairs being installed in serial makes 2x longer as well.
