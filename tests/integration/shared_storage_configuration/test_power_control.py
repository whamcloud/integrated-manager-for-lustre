...
class ChromaPowerControlTestCase(ChromaIntegrationTestCase):
    ...
    def all_outlets_known(self):
        outlets = self.get_list("/api/power_control_device_outlet/", args={"limit": 0})
...
class TestPduSetup(ChromaPowerControlTestCase):
    ...
    @unittest.skipUnless(len(config.get("power_distribution_units", [])), "requires PDUs")
    @unittest.skipUnless(
        config.get("power_control_types", [{}])[0].get("max_outlets", 0), "requires non-IPMI power control"
    )
    ...
    def test_removed_host_disassociated_with_outlets(self):
        ...
        response = self.chroma_manager.put(self.server["resource_uri"], body=self.server)
...
class TestHostFencingConfig(ChromaPowerControlTestCase):
    ...
    def test_saved_outlet_triggers_fencing_update(self):
        ...
        server_outlets = [
            outlet
            for outlet in self.get_list("/api/power_control_device_outlet/")
        ...
        for outlet in server_outlets:
            self.chroma_manager.patch(outlet["resource_uri"], body={"host": None})
        ...
        for outlet in server_outlets:
            self.chroma_manager.patch(outlet["resource_uri"], body={"host": self.server["resource_uri"]})
    ...
    def test_toggled_outlet_does_not_trigger_fencing_update(self):
        def _fencing_job_count():
            return len(
                [
                    j
                    for j in self.get_list("/api/job/", args={"state": "complete"})
