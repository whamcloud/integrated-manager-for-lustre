cleanup() {
    ...
    python $CHROMA_DIR/tests/integration/utils/chroma_log_collector.py $WORKSPACE/test_logs $CLUSTER_CONFIG | tee $WORKSPACE/log_collector_out 2>&1 || true
    ...
    if [ -f $CHROMA_DIR/tests/framework/utils/provisioner_interface/release_cluster ]; then
        $CHROMA_DIR/tests/framework/utils/provisioner_interface/release_cluster || true
    else
        {
        echo "*****************************************************************"
        echo "$CHROMA_DIR/tests/framework/utils/provisioner_interface/release_cluster could not be found"
        if $got_aborted; then
