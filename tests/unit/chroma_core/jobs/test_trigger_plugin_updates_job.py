...
class TestTriggerPluginUpdatesJob(TestJobs):
    ...
    def _test_creation(self, plugin_names):
        test_trigger_job = TriggerPluginUpdatesJob(host_ids=json.dumps(self.host_ids),
        ...
        description_message = 'Trigger plugin poll for %s plugins' % ('all' if plugin_names == [] else ', '.join(plugin_names))
