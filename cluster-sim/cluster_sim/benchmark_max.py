...
class FilesystemSizeLimit(Benchmark):
    def run(self):
        ...
        SU_SIZE = 4
        batch_size = self.args.reg_batch_size
        if batch_size % SU_SIZE != 0:
            raise AssertionError("batch size must be a multipe of %s" % SU_SIZE)
        n = self.args.su_count

        log.debug("Connection count initially: %s" % self._connection_count())

        VOLUMES_PER_SERVER = 4
        while True:
            ost_count = (n * SU_SIZE * VOLUMES_PER_SERVER - 2)
            log.info("SU Count = %s (host count = %s, ost count = %s)" % (n, n * SU_SIZE, ost_count))

            secret = self.get_registration_secret(n * SU_SIZE, datetime.timedelta(seconds=3600))
            fqdns = []
            serials = []

            log.debug("Creating simulated servers...%s SUs (%s servers) (OSTs %s)" % (n, n * SU_SIZE, n * SU_SIZE * VOLUMES_PER_SERVER))
            for i in range(0, n):
            ...
            batch_command_uris = []
            log.debug("Registering servers...%s SUs (%s servers) (OSTs %s)" % (n, len(fqdns), len(serials)))
            with timed("Setup %s SUs (%s servers) (OSTs %s)" % (n, len(fqdns), len(serials))):
                while True:
                    for fqdn_batch in (fqdns[i:i + batch_size] for i in xrange(0, len(fqdns), batch_size)):
                        log.info("Registering batch sized %s" % len(fqdn_batch))
                        for fqdn in fqdn_batch:
                            registration_result = self.simulator.register(fqdn, secret)
                            batch_command_uris.append("/api/command/%s/" % (registration_result['command_id']))

                        try:
                            self._wait_for_commands(batch_command_uris)
                            batch_command_uris = []
                        except RuntimeError, e:
                            log.error("Failed registering %s servers: %s.  Used a smaller --batch-size." % (batch_size, e))
                            raise
                    else:
            ...
            with timed("Filesystem creation POST (Servers %d, %d OSTs)" %
                       (len(fqdns), ost_count)):
            ...
            with timed("Filesystem execution for %s SUs (%s servers) (OSTs %s)" %
                                        (n, len(fqdns), len(serials))):
...
#    server_count_limit_parser.set_defaults(func=lambda args, simulator: ConcurrentRegistrationLimit(args, simulator).run_benchmark())
    ...
    server_count_limit_parser.add_argument('--su_count', type=int, default=10, help='Number of scalable units to benchmark.  Note: SU size is 1 host, 1 controller, and 4 OSTs.')
    server_count_limit_parser.add_argument('--reg_batch_size', type=int, default=8, help='Number of servers to register concurrently.')
